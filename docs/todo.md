...
- [ ] **[BUG] Demo broken in LinkedIn iOS in-app browser:** Tapping "Try the Demo" from inside the LinkedIn iOS in-app browser (WKWebView) shows "The file could not be downloaded. File downloads are not supported." followed by "Something went wrong. We're having issues loading this page." Multiple fixes were attempted (HTML response from `/api/demo/login`, moving handler to non-`/api` path `/demo-start`, adding `Content-Disposition: inline`, `target="_blank"`, in-app browser UA detection + Safari banner) and **all were reverted** because they broke the demo in regular browsers. Root cause appears to be LinkedIn's WKWebView blocking either cross-domain navigation (`hcmcshiftswap.com` → `shiftswapper-demo.vercel.app`) or any programmatic JS navigation. Reverted to last-known-good state at commit `ae82581`. Next attempt should be made in a feature branch with end-to-end testing in both regular browser AND LinkedIn iOS app **before** merging to main. Consider: in-app browser detection that ONLY adds workaround for those UAs, leaving the regular flow untouched.
